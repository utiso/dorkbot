...
def main():
    ...
    if args.help:
        ...
        if args.scanner:
            scanner_parser, _ = get_module_parser(
                load_module("scanners", args.scanner), parent_parser=indexer_parser
            )
    ...
    if (
        ...
        or args.flush_targets
        or args.add_target
        or args.delete_target
        or args.list_blocklist
        or args.flush_blocklist
        or args.add_blocklist_item
        or args.delete_blocklist_item
        or args.flush_fingerprints
        or args.generate_fingerprints
        or args.reset_scanned
        ...
        if args.add_target:
            skip = {
                "skip_on_match": args.delete_on_match,
                "skip_on_error": args.delete_on_error,
            }
            db.add_targets(
                [args.add_target], source=args.source, blocklists=blocklists, **skip
            )
        ...
        if args.indexer:
            ...
            indexer_args = indexer_parser.parse_args(
                format_module_args(args.indexer_arg)
            )
        ...
        if args.scanner:
            ...
            scanner_args = scanner_parser.parse_args(
                format_module_args(args.scanner_arg)
            )
...
def get_initial_args_parser(args=None):
    ...
    initial_parser = argparse.ArgumentParser(description="dorkbot", add_help=False)
    initial_parser.add_argument(
        "-c", "--config", default=config_args.config, help="Configuration file"
    )
    initial_parser.add_argument(
        "-r",
        "--directory",
    ...
    retrieval_options.add_argument(
        "--count",
        type=int,
        default=0,
    ...
    retrieval_options.add_argument(
        "--random", action="store_true", help="retrieve targets in random order"
    )
...
def get_main_args_parser(args=None):
    ...
    database.add_argument("-d", "--database", help="Database file/uri")
    ...
    database.add_argument(
        "--drop-tables", action="store_true", help="Delete and recreate tables"
    )
    database.add_argument(
        "--retries",
        type=int,
    ...
    targets.add_argument(
        "-l", "--list-targets", action="store_true", help="List targets in database"
    )
    targets.add_argument(
        "-n",
        "--unscanned-only",
    ...
    targets.add_argument(
        "--list-sources", action="store_true", help="List sources in database"
    )
    targets.add_argument(
        "--add-target", metavar="TARGET", help="Add a url to the target database"
    )
    targets.add_argument(
        "--delete-target",
        metavar="TARGET",
    ...
    targets.add_argument(
        "--flush-targets", action="store_true", help="Delete all targets"
    )
    targets.add_argument(
        "-m",
        "--delete-on-match",
    ...
    indexing.add_argument("-i", "--indexer", help="Indexer module to use")
    ...
    scanning.add_argument("-s", "--scanner", help="Scanner module to use")
    ...
    scanning.add_argument(
        "--mark-unscanned", help="Reset scanned status of given target"
    )
    ...
    blocklist.add_argument(
        "--list-blocklist", action="store_true", help="List internal blocklist entries"
    )
    blocklist.add_argument(
        "--add-blocklist-item",
        metavar="ITEM",
...
def index(db, blocklists, indexer, args, indexer_args):
    ...
    logging.info(
        f"Indexing: {indexer_name} {vars(indexer_args) if args.verbose else ''}"
    )
...
def scan(db, blocklists, scanner, args, scanner_args):
    ...
    while not args.count or scanned < args.count:
        ...
        target = Target(url)
        logging.info(
            f"Scanning: {target.url} {vars(scanner_args) if args.verbose else ''}"
        )
        ...
        report = generate_report(
            target.url, start_time, end_time, scanner_args.label, results
        )
